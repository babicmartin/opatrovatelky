...
namespace App\UI\Admin\Control\Partner\PartnerDocuments;
...
class PartnerDocumentsControl extends Control
{
	...
	public function setContext(int $partnerId): static
	{
	...
	public function render(): void
	{
		...
		$template->setFile(__DIR__ . '/templates/PartnerDocumentsControl.latte');
		$template->title = $this->title;
		$template->documents = $this->fileRepository->findDocuments($this->dir, $this->partnerId);
		$template->typeImageBasePath = $this->storageDirProvider->getDocumentTypeImages();
		$template->canManagePartner = $this->user->isAllowed(Resource::PARTNER->value);
	...
	public function handleDownload(int $id): void
	{
		...
		$document = $this->fileRepository->findDocument($this->dir, $this->partnerId, $id);
		if ($document === null) {
	...
	public function handleDelete(int $id): void
	{
		...
		$this->changeAuditLogger->logDocumentDeleted('documents.partner', $id, (string) $document['name'], $this->dir, $this->partnerId);
		$this->redirect('this');
	}
	...
	protected function createComponentDocumentForm(): Multiplier
	{
		...
		return new Multiplier(function (string $id): Form {
			$document = $this->fileRepository->findDocument($this->dir, $this->partnerId, (int) $id);
			if ($document === null) {
				$this->getPresenter()->error('Dokument neexistuje.', 404);
			}

			$form = $this->baseFormFactory->create();
			$form->getElementPrototype()->setAttribute('class', 'partner-document-form js-autosave-form');
	...
	protected function createComponentUploadForm(): Form
	{
		...
		$form->onSuccess[] = function (Form $form, ArrayHash $values): void {
			...
			$documentId = $this->fileRepository->insertDocument($this->dir, $this->partnerId, $fileName, $type, (int) $this->user->getId());
			$this->changeAuditLogger->logDocumentUploaded('documents.partner', $documentId, $fileName, $this->dir, $this->partnerId, [
				'file_type' => $type,
				'extension' => $extension,
	...
	{
		return $this->directoryProvider->getRootDir() . '/' . $this->storageDirProvider->getPrivateDocuments() . '/' . $this->dir . '/' . $this->partnerId;
	}
}
