...
- Do not use legacy numeric `permission` filtering in repositories to decide what the current user may see or execute. Repositories should return domain/menu data based on domain filters only; authorization belongs at the UI/action boundary. In Latte templates, render protected links, buttons, and menu items only after checking `$user->isAllowed($resource)` or the equivalent ACL resource for that item. In components, every `handle*()` signal or action that mutates data, deletes rows, exports files, or reveals protected data must also check `$this->presenter->getUser()->isAllowed(...)` before doing the work, even if the button was hidden in the template.
