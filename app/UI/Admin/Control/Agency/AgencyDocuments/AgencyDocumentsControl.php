...
class AgencyDocumentsControl extends Control
{
	...
	public function setContext(int $agencyId): static
	{
	...
	public function render(): void
	{
		...
		$template->setFile(__DIR__ . '/templates/AgencyDocumentsControl.latte');
		$template->title = $this->title;
		$template->documents = $this->fileRepository->findDocuments($this->dir, $this->agencyId);
		$template->typeImageBasePath = $this->storageDirProvider->getDocumentTypeImages();
		$template->canManageAgency = $this->user->isAllowed(Resource::AGENCY->value);
	...
	protected function createComponentDocumentForm(): Multiplier
	{
		...
		return new Multiplier(function (string $id): Form {
			$document = $this->fileRepository->findDocument($this->dir, $this->agencyId, (int) $id);
			if ($document === null) {
				$this->getPresenter()->error('Dokument neexistuje.', 404);
			}

			$form = $this->baseFormFactory->create();
			$form->getElementPrototype()->setAttribute('class', 'agency-document-form js-autosave-form');
	...
	{
		return $this->directoryProvider->getRootDir() . '/' . $this->storageDirProvider->getPrivateDocuments() . '/' . $this->dir . '/' . $this->agencyId;
	}
}
